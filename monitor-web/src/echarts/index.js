...
function defaultOption(name, dataX) {
    return {
        tooltip: {
            trigger: 'axis', //鼠标悬停在数据点上时显示
            position: function (pt) {
                return [pt[0], pt[1]];
            },
            confine: true, //被限制在图表区域内
            padding: 3,
            ...
            axisLabel: {
                formatter: function (value) {
                    ...
                    const date = [value.getDate(), value.getMonth() + 1].join('/')
                    return `${time}\n${date}`;
                }
            }
        }, yAxis: {
    ...
    option.series = [
        {
            ...
            showSymbol: false,
            itemStyle: {
                color: colors[0] //线条的颜色
            },
            areaStyle: { //这个渐变从上到下（0,0,0,1）
